<template>
  <div class="newAddress-page">
    <div v-for="info in infoDatas">
      ...
      <div class="newAddress-user-info">
        <p class="newAddress-user-name border-1px_bottom">收货人</p>
        <div class="newAddress-user-name newAddress-user-input border-1px_bottom">
          <p class="newAddress-user-input"><span class="color505050">姓名</span><input type="text" placeholder="请填写姓名" v-model="info.name"></p>
        </div>
        <p class="newAddress-user-name newAddress-user-input border-1px_bottom newAddress-user-input-width1">
          <span class="color505050">微信号</span><input type="text" placeholder="请填写微信号" v-model="info.weixin_id">
        </p>
        <p class="newAddress-user-name newAddress-user-input border-1px_bottom">
          <span class="color505050">电话</span><input type="text" placeholder="请填写手机号" v-model="info.contacts">
        </p>
        <p class="newAddress-user-name newAddress-user-input border-1px_bottom newAddress-user-input-width">
          <span class="color505050">邮政编码</span><input type="text" placeholder="请填写邮政编码" v-model="info.zip_code">
        </p>
      </div>

      <div class="newAddress-user-info">
        <p class="newAddress-user-name border-1px_bottom">收货地址</p>
        <div class="newAddress-user-name newAddress-user-input border-1px_bottom">
          <p class="newAddress-user-input newAddress-user-input-width">
            <section class="cont" @click="choseAdd()">
              <span class="color505050">所在地区</span><span v-model="address_1">{{Province?Province:''}}</span> <span v-model="address_2">{{City?City:''}}</span> <span v-model="address_3">{{District?District:''}}</span><img width="6" height="10" src="./jiantou2@2x.png">
              <div style="clear: both"></div>
            </section>
          </p>
        </div>
        <p class="newAddress-user-name newAddress-user-input border-1px_bottom newAddress-user-input-height">
          <span class="color505050">详细地址</span><textarea name="input" placeholder="请填写详细街道" v-model="info.address_detail"></textarea>
        </p>
      </div>

      <div class="newAddressEdit" @click="modifySubmit(info)">
        <router-link to="/me/address/modifyNewaddress2">保存</router-link>
      </div>
    </div>

    <!-- 居住地址三级联动选项 -->
    <section class="myAddress" v-show="showChose">
      <section class="myAddress">
        <section class="title title_nav">
          <h4>选择地区</h4>
          <span @click="closeAdd()"><img class="title_img" width="21" height="18" src="./fanhui@2x.png" alt=""></span>
        </section>
        <section class="title border-1px_bottom">
        ...
        <div class="qwe">
          <ul class="zaq">
            <li class="addList border-1px_bottom" v-for="(v,k) in info"
                @click="getProvinceId(v.id, v.name, k)"
                v-show="showProvince"
                :class="v.selected ? 'active' : ''">{{v.name}}</li>
            <li class="addList border-1px_bottom" v-for="(v,k) in showCityList"
                @click="getCityId(v.id, v.name, k)"
                v-show="showCity"
                :class="v.selected ? 'active' : ''">{{v.name}}</li>
            <li class="addList border-1px_bottom" v-for="(v,k) in showDistrictList"
...
<!--<style scoped>
  ...
  .newAddress-user-input1 .marLeft{
    width: 70%;
    display: block;
    float: left;
    height: 16px;
  ...
  .newAddress-user-input>span{
    float: left;
  }
  .user-input-left>span{
    display: block;
    float: left;
  }
  .newAddress-user-input>input,.user-input-left>input{
    display: block;
    float: left;
    width: 60%;
    height: 16px;
  ...
  .newAddress-user-input-width>input{
    width: 70%;
    margin-left: 11px;
  }
  .newAddress-user-input-height{
    height: 90px;
  }
  .newAddress-user-input-height>textarea{
    display: block;
    float: left;
    width: 60%;
    height: 90px;
  ...
  .newAddress-user-input>input::-webkit-input-placeholder{
    color: #c8c8c8;
  }
  .user-input-left>input::-webkit-input-placeholder{
    color: #c8c8c8;
  }
  .newAddress-user-input>textarea::-webkit-input-placeholder{
    font-size: 13px;
    color: #c8c8c8;
    font-family: pingfang;
  }
  ...
  .user-input-right>span,.user-input-right>input{
    display: block;
    float: left;
  }
  .user-input-right>span{
    font-size: 13px;
    color: #505050;
    margin-top: 12px;
    margin-left: 5px;
  }
  .user-input-right>span:nth-child(2){
    margin-right: 30px;
  }
  .check{
    width: 20px;
...
<style scoped>
  ...
  .newAddress-user-input-width1>input{
    width: 70%;
    margin-left: 24px;
  }
...
//*****************************
    ...
    /*created(){
      //接受_兄弟组件数据
      var self = this;
      Bus.$on('getTarget',function(info){
        self.info = info;
//        console.log(info)
      });
    },*/
...
//        console.log(id)
        let _this = this;
        _this.$http.get('/api/getUserAddressDetail',
      ...
      modifySubmit(info){
        ...
        formData.append('idx_address', info.id);
        formData.append('weixin_id', info.weixin_id);
        formData.append('name', info.name);
        formData.append('contacts', info.contacts);
        formData.append('zip_code', info.zip_code);
        formData.append('address_1', info.address_1);
        formData.append('address_2', info.address_2);
        formData.append('address_3', info.address_3);
        formData.append('address_detail', info.address_detail);
        _this.$http.put('/api/updateUserAddress', formData);
//        console.log(info.name);
//        console.log(info.weixin_id);
//        console.log(info.contacts);
//        console.log(info.zip_code);
//        console.log(info.address_1);
//        console.log(info.address_2);
//        console.log(info.address_3);
//        console.log(info.address_detail);
      },
